...
public class HotelCatalogReconciler {
    ...
    public void reconcile() {
        ...
        hotels.forEach(hotel -> outboxEventWriter.write(
                hotel.getId(), EventType.HOTEL_CREATED, payloadFactory.toCatalogPayload(hotel)));
    }
    ...
    public ResyncResult resyncAll() {
        ...
        active.forEach(hotel -> outboxEventWriter.write(
                hotel.getId(), EventType.HOTEL_CREATED, payloadFactory.toCatalogPayload(hotel)));
        withdrawn.forEach(hotel -> outboxEventWriter.write(
                hotel.getId(), EventType.HOTEL_DELETED, new HotelDeletedPayload(hotel.getId())));

        log.warn(
                "Catalog resync: re-emitted {} active (CREATED) + {} withdrawn (DELETED) hotel events",
                active.size(),
                withdrawn.size());
